  ...
  (testing "info message in dev mode"
    (let [config {:dev-mode true}]
      (is (= "info | Some message.\n"
             (with-system-out-str (info config "Some message."))))

      (is (= "info | A message. duration=10ms\n"
             (with-system-out-str (info config "A message." {:duration "10ms"}))))

      (is (= "info | My message. duration=12ms method=GET\n"
             (with-system-out-str (info config "My message." {:duration "12ms"
                                                       :method "GET"}))))

      (is (= "info | Message w/ string attribute. method=POST path=\"/foo\"\n"
             (with-system-out-str (info config "Message w/ string attribute." {:method "POST"
                                                                        :path "/foo"}))))

      (is (= "info | Message w/ nested map attributes. method=GET params={:name \"jdoe\"}\n"
             (with-system-out-str (info config "Message w/ nested map attributes." {:method "GET"
                                                                             :params {:name "jdoe"}}))))))
  ...
  (testing "error message in dev mode"
    (let [config {:dev-mode true}]
      (is (= "error | Some message.\n"
             (with-system-out-str (error config "Some message."))))

      (is (= "error | A message. duration=10ms\n"
             (with-system-out-str (error config "A message." {:duration "10ms"}))))

      (is (= "error | My message. duration=12ms method=GET\n"
             (with-system-out-str (error config "My message." {:duration "12ms"
                                                        :method "GET"}))))

      (is (= "error | Message w/ string attribute. method=POST path=\"/foo\"\n"
             (with-system-out-str (error config "Message w/ string attribute." {:method "POST"
                                                                         :path "/foo"}))))

      (is (= "error | Message w/ nested map attributes. method=GET params={:name \"jdoe\"}\n"
             (with-system-out-str (error config "Message w/ nested map attributes." {:method "GET"
                                                                              :params {:name "jdoe"}}))))))
